...
// The file is completely botched around line 838.
// I will just use a regex to replace everything from `<div className="mt-8 text-center bg-cyber-darker border border-white/5 rounded-3xl p-8 sm:p-12 relative overflow-hidden">`
// up to `// Expired State`. Wait, no, I'll just restore the whole `step === 4` section!

// Let me find the start of step === 4 block.
const startMarker = `          {step === 4 && (`;
const endMarker = `          {/* Navigation Buttons (Steps 1-3) */}`;
...
if (startIndex !== -1 && endIndex !== -1) {
    const newContent = content.substring(0, startIndex) + fix + content.substring(endIndex);
    fs.writeFileSync('src/pages/Book.tsx', newContent);
