...
async function fixSchema() {
  try {
    ...
    const propColl = await pb.collections.create({
      name: "properties",
      type: "base",
      fields: [
    ...
    const propId = propColl.id;

    console.log("Recreating station_types...");
    const stTypeColl = await pb.collections.create({
      name: "station_types",
      type: "base",
      fields: [
        { name: "id", type: "text", required: true, primaryKey: true, system: true },
        ...
        { name: "property_id", type: "relation", required: false, options: { collectionId: propId, maxSelect: 1 } }
      ],
      listRule: "", viewRule: "", createRule: "", updateRule: "", deleteRule: ""
    });

    console.log("Recreating stations...");
    const statColl = await pb.collections.create({
      name: "stations",
      type: "base",
      fields: [
        { name: "id", type: "text", required: true, primaryKey: true, system: true },
    ...
    await pb.collections.create({
      ...
      fields: [
        ...
        { name: "assigned_station_id", type: "relation", required: false, options: { collectionId: statColl.id, maxSelect: 1 } },
        { name: "property_id", type: "relation", required: false, options: { collectionId: propId, maxSelect: 1 } }
      ],
      listRule: "", viewRule: "", createRule: "", updateRule: "", deleteRule: ""
    });
